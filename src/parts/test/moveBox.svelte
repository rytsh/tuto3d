...
import { changeVisibilityOfNonSelectedObjects } from "@/helper/modified/outlinePass";
...
const generateBox = () => {
  ...
  for (let i = 0; i < 1; i++) {
    materials[i] = new THREE.MeshPhongMaterial({ color: "#ff0000" });
...
const catchMouseRClick = (e: MouseEvent) => {
  ...
  if (objectMove) {
    objectClickedColor = "#" + (objectClicked.material as THREE.MeshPhongMaterial).color.getHexString();
  }
  // Check Menu size
  let checkH = (canvas.getBoundingClientRect().height - mouseR.y) - rMenu.getBoundingClientRect().height;
  checkH = checkH >= 0 ? 0 : checkH;
...
const catchMouseClick = (e: MouseEvent) => {
  ...
  // console.log("Mouse clicked", e.button, objectnow);
  if ((e.button - 2) < 0) {
...
const colorChange = (color: string) => {
  if (objectClicked) {
    (objectClicked.material as THREE.MeshPhongMaterial).color.setStyle(color);
  }
};
...
onMount(() => {
  ...
  transformControl.traverse((obj:any) => { // To be detected correctly by OutlinePass.
    obj.isTransformControl = true;
  });
  ...
  outlinePass.changeVisibilityOfNonSelectedObjects = changeVisibilityOfNonSelectedObjects;
